// ===== App config =====
const SECTION_META = {
  1: { title: '一、單一選擇題', desc: '單選題，共 57 題，10 題為一份', color: 1 },
  2: { title: '二、多重選擇題', desc: '多選題，共 35 題，10 題為一份', color: 2 },
  3: { title: '三、題組', desc: '閱讀資料後回答多個子題（單選／多選），共 11 組，每組算 1 題，10 組為一份', color: 3 },
  4: { title: '四、閱讀測驗', desc: '閱讀資料後回答多個子題（單選／多選），共 16 組，每組算 1 題，10 組為一份', color: 1 },
  5: { title: '五、混合題', desc: '閱讀資料後回答選擇＋問答題，共 12 組，每組算 1 題，10 組為一份', color: 2 },
  6: { title: '六、素養題', desc: '閱讀資料後回答選擇＋問答題，共 5 組', color: 3 },
};
...
function buildQuestionCard(q, state, scoreBoard) {
  ...
  if (q.type === 'single' || q.type === 'multi') {
    ...
    q.parts.forEach(part => {
      ...
      buildMcBlock(sub, q, part.options, part.answer, !!part.multi, state, scoreBoard, null);
...
function buildTextBlock(container, part) {
  ...
  revealBtn.addEventListener('click', () => {
    ...
    revealBtn.textContent = showing ? '✅ 確認答案' : '🙈 隱藏參考答案';
  });
}
